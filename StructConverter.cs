...
namespace CamControl
{
    ...
    {
        public static byte[] MergeBytes(params byte[][] ByteArrays)
        {
            int TotalLenth = 0;
            foreach (byte[] item in ByteArrays) TotalLenth = TotalLenth + item.Length;
            int CopiedBytes = 0;
            byte[] p = new byte[TotalLenth];
            foreach (byte[] item in ByteArrays)
            {
                item.CopyTo(p, CopiedBytes);
        ...
        public static byte[] Pack(object[] items, bool LittleEndian, out string NeededFormatStringToRecover)
        {
            ...
            foreach (object o in items)
            {
                if (o is byte[])
                {
                    foreach (var _o in o as byte[])
                    {

                        byte[] theseBytes = TypeAgnosticGetBytes(_o);
                        if (endianFlip == true) theseBytes = (byte[])theseBytes.Reverse();
                        outString += GetFormatSpecifierFor(_o);
        ...
        public static byte[] Pack(params object[] items)
        {
            string dummy = "";
